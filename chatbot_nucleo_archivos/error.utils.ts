...
export interface AppError {
   ...
   */
  export function createAppError(statusCode: number, message: string, details?: any): AppError {
  ...
  export function toAppError(error: unknown): AppError {
    ...
    return createAppError(500, 'Error interno del servidor', { originalError: error });
  }
  ...
  export function handleStorageError(error: unknown): AppError {
    ...
    if (error instanceof Error) {
      ...
      if (errorCode === 404 || errorCode === 'ResourceNotFound') {
        return createAppError(404, 'Recurso no encontrado', { originalError: error.message });
      }
      
      if (errorCode === 409 || errorCode === 'EntityAlreadyExists') {
        return createAppError(409, 'El recurso ya existe', { originalError: error.message });
      }
