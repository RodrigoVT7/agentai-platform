...
import { MessageRequest, MessageType, MessageStatus, TempUserInfo } from "../../models/conversation.model"; // Asegúrate que estén importados
...
export class WhatsAppIntegrationHandler {
  ...
  public async sendMessage(
    messageData: { // Tipo más específico para los datos del mensaje
    ...
    try {
        ...
        if (!integration) {
         ...
         if (integration.status !== IntegrationStatus.ACTIVE) {
             return { status: 400, jsonBody: { error: `La integración WhatsApp (${integration.name}) no está activa.` } };
         }
        ...
        switch (type) {
            case 'text':
                if (!messageData.text || !messageData.text.body) {
                    return { status: 400, jsonBody: { error: "Falta el cuerpo del mensaje de texto (text.body)." } };
                }
                payload.text = messageData.text;
                break;
            case 'template':
                if (!messageData.template || !messageData.template.name || !messageData.template.language?.code) {
                    return { status: 400, jsonBody: { error: "Faltan datos de la plantilla (name, language.code)." } };
                }
                payload.template = messageData.template;
                break;
             case 'image':
                 if (!messageData.image || (!messageData.image.id && !messageData.image.link)) {
                     return { status: 400, jsonBody: { error: "Se requiere ID o link para la imagen." } };
                 }
                 payload.image = messageData.image;
                 break;
             case 'document':
                 if (!messageData.document || (!messageData.document.id && !messageData.document.link)) {
                      return { status: 400, jsonBody: { error: "Se requiere ID o link para el documento." } };
                 }
                  // Se requiere filename si se usa link
                 if (messageData.document.link && !messageData.document.filename) {
                      return { status: 400, jsonBody: { error: "Se requiere 'filename' cuando se usa 'link' para documentos." } };
                 }
                 payload.document = messageData.document;
                 break;
             case 'interactive':
                 if (!messageData.interactive) {
                     return { status: 400, jsonBody: { error: "Faltan datos interactivos." } };
                 }
            ...
            default:
                return { status: 400, jsonBody: { error: `Tipo de mensaje no soportado para envío: ${type}` } };
        }

        // 4. Enviar Petición a la API Graph de Meta
        this.logger.info(`Enviando mensaje a ${to} via ${config.phoneNumberId}. Payload: ${JSON.stringify(payload)}`);
        const response = await fetch(
        ...
        if (!response.ok) {
            this.logger.error(`Error al enviar mensaje WA (${response.status}): ${JSON.stringify(responseBody)}`);
        ...
        this.logger.info(`Respuesta de envío WA: ${JSON.stringify(responseBody)}`);
        const waMessageId = responseBody.messages?.[0]?.id; // Obtener el ID del mensaje de WhatsApp

        // 6. Registrar Éxito y Devolver Respuesta
        this.logMessageSent(integration.agentId, integrationId, to, type, waMessageId || 'N/A');
        ...
        this.logger.error(`Error fatal en sendMessage para integración ${messageData.integrationId}:`, error);
        return {
            status: 500,
  ...
  public async processWebhook(webhookData: any): Promise<void> {
    ...
    try {
      if (!webhookData.object || !webhookData.entry || webhookData.entry.length === 0) {
      ...
      for (const entry of webhookData.entry) {
        ...
        for (const change of entry.changes) {
          ...
          if (!phoneNumberId) {
            this.logger.warn("No se encontró phoneNumberId en los metadatos del webhook.");
            continue;
          }
          ...
          if (!integration) {
            this.logger.warn(`Webhook recibido para un número no configurado o inactivo: ${phoneNumberId}`);
      ...
      this.logger.error("Error detallado al procesar webhook de WhatsApp:", error);
      // No lanzar error para que WhatsApp no reintente indefinidamente
    }
  }

  private async processStatusUpdate(integration: Integration, statusUpdate: any): Promise<void> {
    ...
    this.logger.info(`Actualización de estado recibida para mensaje WA ${messageId}: ${status}`);
    ...
    try {
        ...
        const messages = messagesTableClient.listEntities({ queryOptions: { filter } });
        ...
        for await (const message of messages) {
            ...
            switch (status) {
                ...
                default: newStatus = message.status as MessageStatus; // No cambiar si es desconocido
            }
            ...
            const currentStatusIndex = statusOrder.indexOf(message.status as MessageStatus);
            ...
            if (newStatus === MessageStatus.FAILED || newStatusIndex > currentStatusIndex) {
                ...
                this.logger.info(`Estado del mensaje interno ${internalMessageId} actualizado a ${newStatus}`);
                 ...
                 this.logger.info(`Estado ${status} ignorado para mensaje ${internalMessageId} (estado actual: ${message.status})`);
            }
        ...
        if (!foundMessage) {
            this.logger.warn(`No se encontró el mensaje interno correspondiente al ID de WhatsApp ${messageId}`);
        }

    } catch (error) {
        this.logger.error(`Error al procesar actualización de estado para mensaje WA ${messageId}:`, error);
        // No relanzar
    }
}
  ...
  try {
    ...
    switch (message.type) {
      ...
      case 'image':
        ...
        attachments = { image: { id: message.image?.id, mime_type: message.image?.mime_type } };
        this.logger.info(`Imagen recibida: ${message.image?.id}`);
        break;
      case 'document':
        content = message.document?.caption || '[Documento Recibido]';
        messageType = MessageType.FILE;
        attachments = { document: { id: message.document?.id, filename: message.document?.filename, mime_type: message.document?.mime_type } };
        this.logger.info(`Documento recibido: ${message.document?.filename}`);
        break;
      case 'audio':
        content = '[Audio Recibido]';
        messageType = MessageType.AUDIO;
        attachments = { audio: { id: message.audio?.id, mime_type: message.audio?.mime_type } };
      ...
      case 'interactive':
        if (message.interactive?.button_reply) {
          ...
          messageType = MessageType.TEXT;
          this.logger.info(`Respuesta de botón recibida: ${content} (ID: ${message.interactive.button_reply.id})`);
        } else if (message.interactive?.list_reply) {
          content = message.interactive.list_reply.title;
          messageType = MessageType.TEXT;
          this.logger.info(`Respuesta de lista recibida: ${content} (ID: ${message.interactive.list_reply.id})`);
        } else {
          this.logger.warn("Mensaje interactivo desconocido:", message.interactive);
          return;
        }
        break;
      default:
        this.logger.warn(`Tipo de mensaje de WhatsApp no manejado: ${message.type}`);
        return;
    }

    if (!content && !attachments) {
      this.logger.warn(`Mensaje de tipo ${message.type} sin contenido procesable.`);
      return;
    }
    ...
    const messageRequest: MessageRequest = {
            ...
            metadata: {
                whatsapp: {
                    from: from,
                    fromName: profileName, // Solo nombre del perfil de WhatsApp
                    waMessageId: waMessageId,
                    timestamp: timestamp,
                    integrationId: integration.id
                },
                attachments: attachments
                // ELIMINADO: extractedInfo - no extraer información sensible
            }
        };
    ...
    this.logger.info(`Mensaje entrante de WhatsApp ${waMessageId} procesado y encolado.`);

  } catch (error) {
    this.logger.error(`Error al procesar mensaje entrante de WhatsApp ${message?.id}:`, error);
  }
}
