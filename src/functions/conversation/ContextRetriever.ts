...
export async function ContextRetriever(
  ...
  try {
    ...
    logger.info(
      `Procesando contexto para mensaje ${message.messageId} en conversación ${message.conversationId}`
    );
    ...
    logger.info(
      `Contexto generado para mensaje ${message.messageId}. Encolado para respuesta.`
    );
  } catch (error) {
