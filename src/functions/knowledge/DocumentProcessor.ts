...
export async function documentProcessor(
  ...
  try {
    ...
    logger.info(
      `Procesando documento ${message.documentId} para base de conocimiento ${message.knowledgeBaseId}`
    );
    ...
    logger.info(
      `Documento ${message.documentId} procesado correctamente. Generados ${result.chunks.length} chunks.`
    );
  } catch (error) {
