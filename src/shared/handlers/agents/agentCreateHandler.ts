...
export class AgentCreateHandler {
  ...
  async execute(agentData: any): Promise<any> {
    try {
      ...
      for await (const agent of existingAgents) {
        if (agent.code === code) {
          throw createAppError(
            409,
            `Ya existe un agente con un nombre similar. Por favor elige otro nombre.`
          );
        }
      }
      ...
      if (agentCount >= maxAgents) {
        throw createAppError(
          403,
          `Has alcanzado el límite de ${maxAgents} agentes en tu plan actual.`
        );
      }
      ...
      const newAgent: Agent = {
        ...
        modelConfig:
          typeof agentData.modelConfig === "object"
            ? JSON.stringify(agentData.modelConfig)
        ...
        operatingHours: agentData.operatingHours
          ? JSON.stringify(agentData.operatingHours)
          : null,
        createdAt: now,
      };
  ...
  private async countUserAgents(userId: string): Promise<number> {
    try {
      ...
      this.logger.error(
        `Error al contar agentes del usuario ${userId}:`,
        error
      );
      return 0;
    }
  ...
  private async createDefaultKnowledgeBase(
    agentId: string,
    userId: string
  ): Promise<string> {
    try {
      ...
      this.logger.error(
        `Error al crear base de conocimiento predeterminada:`,
        error
      );
      throw createAppError(
        500,
