...
export class ChatCompletionHandler {
  ...
  async execute(request: CompletionRequest): Promise<void> {
    ...
    try {
      ...
      const { messages, latestUserQuery } =
        this.prepareCompletionMessages(context);

      if (!latestUserQuery) {
        this.logger.warn(
          `No se encontró una pregunta de usuario válida reciente para el mensaje ${messageId}. Abortando completación.`
        );
        ...
        await this.updateMessageStatus(
          ...
          "No se pudo identificar la pregunta del usuario."
        );
        return;
      }
      ...
      await this.updateUsageStats(
        agentId,
        userId,
        Math.ceil(inputTokensEstimate),
        response.length
      );

      this.logger.info(
        `Response generated and saved for message ${messageId}.`
      );

      return;
    } catch (error) {
      this.logger.error(
        `Error al generar respuesta para mensaje ${messageId}:`,
        error
      );
    ...
    try {
      ...
      if (typeof agent.modelConfig === "string" && agent.modelConfig) {
        try {
          ...
          this.logger.warn(
            `Error al parsear modelConfig para agente ${agentId}:`,
            e
          );
        }
      } else if (
      ...
      this.logger.error(
        `Error al obtener configuración del agente ${agentId}. Usando defaults.`,
        error
      );
      return {
   ...
   */
  private prepareCompletionMessages(context: ContextResult): {
    ...
    const { recentValidMessages, latestUserMessage } =
      this.getRecentValidMessages(
        context.conversationContext,
        MAX_RECENT_MESSAGES
      );
    const latestUserQuery = latestUserMessage
      ? latestUserMessage.content
      : null;

    this.logger.info(
      `Última pregunta de usuario identificada: ${
        latestUserQuery ? `"${latestUserQuery.substring(0, 50)}..."` : "Ninguna"
      }`
    );

    // 2. Construir el prompt del sistema
    let systemContent =
      context.systemInstructions || "You are a helpful assistant.";
    ...
    if (context.activeIntegrations && context.activeIntegrations.length > 0) {
      ...
      systemContent +=
        "You currently have the following active integrations:\n";
      ...
      systemContent +=
        "You can use these integrations if the conversation requires it.\n";
      systemContent += "### End of Capabilities\n";
    } else {
      systemContent +=
        "\n\nYou currently have no active external integrations.\n";
    }
    ...
    if (context.relevantChunks && context.relevantChunks.length > 0) {
      ...
      context.relevantChunks.forEach((chunk, index) => {
        if (chunk.similarity > 0.7) {
          systemContent += `--- INICIO Documento ${
            index + 1
          } (Similitud: ${chunk.similarity.toFixed(2)}) ---\n${
            chunk.content
    ...
    if (latestUserQuery) {
      messages.push({
        role: "system",
        content: `Please provide a direct and concise answer to the user's last question: "${latestUserQuery}"`,
      });
    } else {
      ...
      if (lastMessage) {
        messages.push({
          role: "system",
          content: `Consider the last message in the conversation (${
            lastMessage.role
        ...
        messages.push({
          role: "system",
          content:
            "Start the conversation or respond generally if there is no previous message.",
        });
      }
    }
  ...
  private getRecentValidMessages(
    ...
    const validMessages: Message[] = conversationContext
      ...
      .filter(
        (msg) =>
          ...
          msg.content &&
          msg.content.trim() !== "" // Excluir mensajes vacíos
      )
      .sort((a, b) => {
    ...
    const latestUserMessage =
      [...validMessages] // Clonar para no afectar el orden original al revertir
        .reverse()
  ...
  private mapRoleToOpenAI(role: MessageRole): "system" | "user" | "assistant" {
    switch (role) {
      ...
      case MessageRole.HUMAN_AGENT: // Tratar agentes humanos como usuarios para el modelo AI
        return "user";
      default:
        this.logger.warn(`Unknown message role mapped to 'user': ${role}`);
  ...
  private async saveAssistantMessage(
    ...
    try {
      ...
      this.logger.debug(
        `Respuesta del asistente guardada como mensaje ${messageId}`
      );
    } catch (error) {
      this.logger.error(
        `Error al guardar mensaje de asistente para conversación ${conversationId}:`,
        error
      );
      // No lanzar error aquí directamente, el error principal se manejará en execute()
      throw createAppError(
        500,
        "Error interno al guardar la respuesta del asistente."
      );
    }
  }
  ...
  private async updateMessageStatus(
    ...
    try {
      ...
      this.logger.debug(
        `Estado del mensaje ${messageId} actualizado a ${status}`
      );
    } catch (error: any) {
      // Evitar error si la entidad no se encuentra (podría haber sido eliminada)
      if (error.statusCode !== 404) {
        this.logger.warn(
          `Error al actualizar estado del mensaje ${messageId} a ${status}:`,
          error
        );
      }
  ...
  private async updateUsageStats(
    ...
    try {
      ...
      const statId = `${today.getFullYear()}-${(today.getMonth() + 1)
        .toString()
        .padStart(2, "0")}-${today.getDate().toString().padStart(2, "0")}`;

      const tableClient = this.storageService.getTableClient(
        STORAGE_TABLES.USAGE_STATS
      );
        ...
        await tableClient.updateEntity(
          {
            ...
            outputTokens:
              (Number(existingStat.outputTokens) || 0) + outputTokens,
            totalTokens: (Number(existingStat.totalTokens) || 0) + totalTokens,
            processedMessages:
              (Number(existingStat.processedMessages) || 0) + 1,
            updatedAt: Date.now(),
          },
          "Merge"
        );
      ...
      this.logger.debug(
        `Estadísticas de uso actualizadas para agente ${agentId}, usuario ${userId}`
      );
    } catch (error) {
      this.logger.warn(
        `Error al actualizar estadísticas de uso para agente ${agentId}:`,
        error
      );
      // No propagar error
    }
