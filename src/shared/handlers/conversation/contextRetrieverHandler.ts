...
export class ContextRetrieverHandler {
  ...
  async execute(message: QueueMessage): Promise<void> {
    ...
    try {
      ...
      const relevantChunks = await this.searchRelevantContent(
        agentId,
      ...
      this.logger.error(
        `Error al procesar contexto para mensaje ${messageId}:`,
        error
      );
  ...
  private async getActiveIntegrations(
    ...
    try {
      ...
      this.logger.debug(
        `Encontradas ${activeIntegrations.length} integraciones activas para el agente ${agentId}`
      );
    } catch (error) {
      this.logger.error(
        `Error al obtener integraciones activas para el agente ${agentId}:`,
        error
      );
  ...
  private async getConversationHistory(
    ...
    try {
      ...
      this.logger.debug(
        `Recuperados ${sortedMessages.length} mensajes para conversación ${conversationId}`
      );

      // Registrar resumen de los mensajes para debugging
      this.logger.debug(
        `Secuencia de mensajes: ${sortedMessages
          .map((msg) => `${msg.id.substring(0, 6)}(${msg.role})`)
          .join(" -> ")}`
      );
      ...
      this.logger.error(
        `Error al obtener historial de conversación ${conversationId}:`,
        error
      );
      return [];
    }
  ...
  private async getAgentSystemInstructions(agentId: string): Promise<string> {
    try {
      ...
      this.logger.error(
        `Error al obtener instrucciones del agente ${agentId}:`,
        error
      );
      return "";
    }
  ...
  > {
    try {
      ...
      const kbTableClient = this.storageService.getTableClient(
        STORAGE_TABLES.KNOWLEDGE_BASES
      );
      let knowledgeBaseId: string | null = null;
      const kbs = await kbTableClient.listEntities({
        queryOptions: {
          filter: `PartitionKey eq '${agentId}' and isActive eq true`,
        },
      });
      ...
      if (!knowledgeBaseId) {
        this.logger.warn(
          `No se encontró base de conocimiento activa para el agente ${agentId}`
        );
        return [];
      }
      ...
      const searchResults = await this.documentSearchHandler.execute(
        searchParams
      );
      ...
      this.logger.error(
        `Error al buscar contenido relevante para agente ${agentId} usando AI Search:`,
        error
      );
      return [];
    }
  ...
  private async queueForCompletion(
    ...
    try {
      ...
      await queueClient.sendMessage(
        Buffer.from(JSON.stringify(completionRequest)).toString("base64")
      );

      this.logger.debug(
        `Mensaje ${messageId} encolado para generación de respuesta`
      );
    } catch (error) {
      this.logger.error(
        `Error al encolar mensaje ${messageId} para completación:`,
        error
      );
