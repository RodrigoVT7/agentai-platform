...
export class ConversationSearchHandler {
  ...
  async execute(searchParams: ConversationSearchParams, userId: string): Promise<SearchResult> {
    try {
      ...
      if (searchParams.agentId) {
        const hasAccess = await this.verifyAgentAccess(searchParams.agentId, userId);
      ...
      if (searchParams.query) {
        conversations = await this.filterConversationsByMessageContent(conversations, searchParams.query);
      }
      ...
      const enrichedConversations = await this.enrichConversations(paginatedConversations);
  ...
  private async findConversations(searchParams: ConversationSearchParams, userId: string): Promise<Conversation[]> {
    try {
      ...
      for await (const entity of conversationsEntities) {
        ...
        if (!searchParams.agentId) {
          ...
          if (entity.userId !== userId) {
            const hasAccess = await this.verifyAgentAccess(entity.agentId as string, userId);
  ...
  private async filterConversationsByMessageContent(conversations: Conversation[], query: string): Promise<Conversation[]> {
    try {
      ...
      return conversations.filter(conv => filteredConversationIds.has(conv.id));
    } catch (error) {
      this.logger.error(`Error al filtrar conversaciones por contenido:`, error);
      return conversations; // En caso de error, devolver sin filtrar
    }
  }
  
  private async enrichConversations(conversations: Conversation[]): Promise<Conversation[]> {
    try {
      if (conversations.length === 0) return [];
      
      const messageTableClient = this.storageService.getTableClient(STORAGE_TABLES.MESSAGES);
