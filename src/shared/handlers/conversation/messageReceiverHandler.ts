...
export class MessageReceiverHandler {
  ...
  async execute(
    ...
    try {
      const {
        ...
        metadata,
        sourceChannel,
      } = messageData; // sourceChannel ahora existe
      ...
      if (sourceChannel === "whatsapp" && metadata?.whatsapp?.from) {
        ...
        endUserId = await this.findOrCreateEndUser(
          `whatsapp:${whatsappNumber}`,
          metadata.whatsapp.fromName
        );

        if (!conversationToUseId) {
          existingConversation = await this.findActiveConversation(
            agentId,
            endUserId,
            sourceChannel
          );
          if (existingConversation) {
            ...
            this.logger.info(
              `Reanudando conversación existente ${conversationToUseId} para ${whatsappNumber}`
            );
          }
        }
      } else {
        ...
        if (!hasAccess) {
          throw createAppError(
            403,
            "No tienes permiso para enviar mensajes a este agente"
          );
        }
      }
      ...
      if (!conversationToUseId) {
        ...
        conversationToUseId = await this.createNewConversation(
          agentId,
          endUserId,
        ...
        existingConversation = await this.findConversationById(
          conversationToUseId
        );
        if (
          ...
          throw createAppError(
            400,
            `La conversación ${conversationToUseId} no existe o no está activa.`
          );
        }
        if (
          existingConversation.endUserId &&
          existingConversation.endUserId !== endUserId
        ) {
          this.logger.warn(
            `Intento de añadir mensaje a conversación ${conversationToUseId} de otro usuario (${existingConversation.endUserId} vs ${endUserId})`
          );
      ...
      this.logger.info(
        `MessageReceiverHandler: PREPARANDO para encolar en '${
          STORAGE_QUEUES.CONVERSATION
        }'. Payload: ${JSON.stringify(queuePayload)}`
      );

      try {
        await queueClient.sendMessage(
          Buffer.from(JSON.stringify(queuePayload)).toString("base64")
        );
        // NUEVO LOG: Log DESPUÉS y en caso de ÉXITO
        this.logger.info(
          `MessageReceiverHandler: ÉXITO al encolar en '${STORAGE_QUEUES.CONVERSATION}'. MessageId encolado: ${messageId}`
        );
      } catch (enqueueError) {
        // NUEVO LOG: Log en caso de FALLO al encolar
        this.logger.error(
          `MessageReceiverHandler: FALLO AL ENCOLAR en '${STORAGE_QUEUES.CONVERSATION}'. MessageId: ${messageId}. Error:`,
          enqueueError
        );
        ...
        throw createAppError(
          500,
          `Fallo al encolar mensaje para procesamiento: ${
            (enqueueError as Error).message
          }`,
          enqueueError
        );
      }
      ...
      this.logger.info(
        `MessageReceiverHandler: Mensaje ${messageId} (origen: ${sourceChannel}) procesado y encolado (según lógica previa).`
      );
  ...
  private async findOrCreateEndUser(
    ...
    const usersTableClient = this.storageService.getTableClient(
      STORAGE_TABLES.USERS
    ); // O una tabla END_USERS
    const partitionKey = "endUser"; // O una clave adecuada

    try {
      // Intentar buscar por RowKey (el identificador único)
      const existingUser = await usersTableClient.getEntity(
        partitionKey,
        endUserIdentifier
      );
      ...
      if (profileName && existingUser.firstName !== profileName) {
        await usersTableClient.updateEntity(
          { partitionKey, rowKey: endUserIdentifier, firstName: profileName },
          "Merge"
        );
      }
      ...
      if (error.statusCode === 404) {
        ...
        this.logger.info(
          `Creando nuevo usuario final para ${endUserIdentifier}`
        );
        ...
        this.logger.error(
          `Error al buscar/crear usuario final ${endUserIdentifier}:`,
          error
        );
  ...
  private async findActiveConversation(
    ...
    try {
      ...
      for await (const conv of conversations) {
        ...
        if (
          !latestConversation ||
          (conv.createdAt as number) > (latestConversation.createdAt as number)
        ) {
      ...
      this.logger.error(
        `Error buscando conversación activa para agente ${agentId} y usuario ${endUserId}:`,
        error
      );
      return null;
    }
  ...
  private async findConversationById(
    ...
    try {
      ...
      this.logger.error(
        `Error buscando conversación por ID ${conversationId}:`,
        error
      );
      return null;
    }
  ...
  private async createNewConversation(
    ...
    try {
      ...
      const now = Date.now();
      const code = `conv-${now.toString(36)}-${Math.random()
        .toString(36)
        .substr(2, 4)}`;
      ...
      this.logger.info(
        `Nueva conversación ${conversationId} creada para agente ${agentId} y usuario final ${endUserId}`
      );
      return conversationId;
    } catch (error) {
      this.logger.error(
        `Error al crear nueva conversación para agente ${agentId}:`,
        error
      );
  ...
  private async updateConversationTimestamp(
    ...
    try {
      ...
      if (error.statusCode !== 404) {
        // Ignorar si la conversación no se encuentra (podría ser un mensaje inicial)
        this.logger.warn(
          `Error al actualizar timestamp de conversación ${conversationId} (agente ${agentId}):`,
          error
        );
      }
    }
  ...
  private async isConversationActive(conversationId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Error al verificar estado de conversación ${conversationId}:`,
        error
      );
      return false;
    }
