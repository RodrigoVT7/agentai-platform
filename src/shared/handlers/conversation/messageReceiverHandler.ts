...
export class MessageReceiverHandler {
  ...
  async execute(
    ...
    try {
      ...
      if (!activeConversationId) {
        activeConversationId = await this.createNewConversation(
          agentId,
          userId,
          metadata
        );
        ...
        if (!isActive) {
          throw createAppError(
            400,
            "No se puede enviar mensaje a una conversación finalizada"
          );
        }
      }
      ...
      this.logger.info(
        `Mensaje ${messageId} recibido y encolado para procesamiento`
      );
  ...
  private async updateConversationTimestamp(
    ...
    try {
      ...
      this.logger.warn(
        `Error al actualizar timestamp de conversación ${conversationId}:`,
        error
      );
