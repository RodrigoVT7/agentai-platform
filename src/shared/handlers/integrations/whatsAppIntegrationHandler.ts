  ...
  async processWebhook(webhookData: any): Promise<void> {
    try {
      ...
      for (const change of changes) {
        ...
        if (!integration) {
          this.logger.warn(
            `No se encontró integración para phoneNumberId: ${phoneNumberId}`
          );
          continue;
        }
  ...
  private async processIncomingMessage(
    ...
    try {
      ...
      await queueClient.sendMessage(
        Buffer.from(JSON.stringify(messageRequest)).toString("base64")
      );

      this.logger.info(
        `Mensaje de WhatsApp encolado para procesamiento: ${message.id}`
      );
    } catch (error) {
      this.logger.error(
        `Error al procesar mensaje entrante de WhatsApp:`,
        error
      );
    }
  }
