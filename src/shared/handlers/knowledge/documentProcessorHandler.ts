      ...
      switch (contentType) {
        ...
        default:
          ...
          if (fileName) {
            ...
            if (extension === '.pdf') return await this.extractTextFromPdf(buffer);
            if (extension === '.txt') return buffer.toString('utf-8');
            if (extension === '.md') return buffer.toString('utf-8');
            if (extension === '.doc' || extension === '.docx') return await this.extractTextFromWord(buffer);
            if (extension === '.csv') return await this.extractTextFromCsv(buffer);
            if (extension === '.json') return await this.extractTextFromJson(buffer);
          }
